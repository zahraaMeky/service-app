...
const BusinessList: React.FC<BusinessListProps> = ({ businessList, title }) => {
    return (
        <section id="services" className="my-5 padding-container">
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-5'>
                {businessList.length > 0 ? businessList.map((business, index) => (
                    <Link href={'/details/' + business.id} key={index} className='shadow-md rounded-lg hover:shadow-lg cursor-pointer hover:shadow-purple-400 hover:scale-105 transition-all ease-in-out'>
                        <Image
                    ...
                    [1, 2, 3, 4, 5, 7, 8, 9].map((item, index) => (
                        <div key={index} className='h-[300px] w-full bg-purple-200 animate-pulse rounded-lg'>
                        </div>
                    ))
                }
            </div>
